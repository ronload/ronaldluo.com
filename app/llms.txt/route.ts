...
function buildEn() {
  return [
    ...
    `- Also known online as: ${PERSON.handles.join(", ")}`,
    "",
    ...
    `- Site: ${SITE_URL}`,
    `- FAQ: ${SITE_URL}/faq`,
    `- Notes: ${SITE_URL}/notes`,
    `- Notes feed (Atom): ${SITE_URL}/feed.xml`,
...
function buildZh() {
  return [
    ...
    `- 網站：${SITE_URL}`,
    `- 常見問題：${SITE_URL}/faq`,
    `- 筆記：${SITE_URL}/zh-TW/notes`,
    `- 筆記訂閱（Atom）：${SITE_URL}/zh-TW/feed.xml`,
    ...links.map((social) => `- ${social.label}：${social.url}`),
    "",
    "## 引用方式",
